# 🐰 Desert Hopper

**AziMeow_The_rabbit_game** - A trap platformer game featuring a cute Minecraft-style rabbit escaping through desert-themed levels!

![Desert Hopper Banner](https://img.shields.io/badge/Desert%20Hopper-1.0-blue?style=for-the-badge)
![License](https://img.shields.io/badge/License-MIT-green?style=for-the-badge)
![Platform](https://img.shields.io/badge/Platform-HTML5-orange?style=for-the-badge)

---

## 🎮 Game Overview

**Desert Hopper** is a challenging trap platformer where you control a cute Minecraft-style rabbit trying to escape through 10 increasingly difficult desert levels. Watch out for hidden traps, moving walls, gravity flips, and troll mechanics that will test your patience and reflexes!

### 🌟 Features

- **10 Unique Levels** - Each level has different trap mechanics and surprises
- **Cute Minecraft-style Graphics** - Boxy rabbit character, sandstone platforms, cactus obstacles
- **Touch & Keyboard Controls** - Play on desktop or mobile
- **Challenging Troll Mechanics** - Expect the unexpected!
- **Death Counter** - Track how many times you've fallen
- **Performance Rating** - Rate yourself based on total deaths

---

## 🎯 How to Play

### Controls

| Input | Action |
|-------|--------|
| **Arrow Keys** or **WASD** | Move left/right |
| **Space** or **W** or **Up Arrow** | Jump |
| **R** | Restart current level |

### Mobile Controls

- **Left Touch Button** - Move left
- **Right Touch Button** - Move right  
- **Jump Button** - Jump

### Objective

1. Navigate your rabbit from the **START** platform to the **EXIT** (sandstone temple door)
2. Avoid **CACTI** traps and other hidden obstacles
3. Complete all 10 levels to win!
4. Aim for the **LOWEST death count** for the best rating

---

## 🏆 Scoring & Ratings

Your final rating is based on your total death count across all 10 levels:

| Deaths | Rating | Message |
|--------|--------|---------|
| **0** | 🏆 PERFECT | "The smartest rabbit ever!" |
| **1-4** | ⭐ Impressive | "True desert hopper!" |
| **5-19** | 👍 Not Bad | "Learning the sands!" |
| **20+** | 😅 Better Luck | "Better luck in the next desert!" |

---

## 🦗 Level Guide

| Level | Name | Challenge |
|-------|------|-----------|
| 1 | Welcome Fool | Hidden spike near exit |
| 2 | Gravity Lied | Gravity flip trap |
| 3 | The Helpful Button | Pressure plate trapdoor |
| 4 | Closing Time | Moving walls closing in |
| 5 | Trust Issues | Fake platforms that fall |
| 6 | Speed Kills | Speed boost danger |
| 7 | Countdown | Timed pressure plate |
| 8 | Mirror Mirror | Inverted controls tile |
| 9 | Almost There | Running exit |
| 10 | Final Boss | Multiple troll mechanics |

---

## 🔧 Level Mechanics

### Traps & Obstacles

- **Cacti** - Sharp desert plants (instant death on contact)
- **Invisible Spikes** - Hidden traps that appear when you get close
- **Fake Platforms** - Look solid but crumble when stepped on
- **Gravity Flips** - Gravity reverses direction
- **Moving Walls** - Walls that close in on you
- **Speed Pads** - Golden pads that boost your speed
- **Pressure Plates** - Stand on them to trigger events
- **Trapdoors** - Floor sections that open when triggered
- **Mirror Tiles** - Invert your left/right controls
- **Moving Exit** - The exit door runs away from you!
- **Fake Win Screens** - Decoy victory screens in Level 10

### Tips

- **Always be suspicious** - If something looks too easy, it's a trap!
- **Watch your step** - Platforms may not be what they seem
- **Don't trust the exit** - In later levels, the exit is not always where it appears
- **Stay still to win** - Level 10 requires you to stand still!

---

## 💀 Death Taunts

When you die, the rabbit will be greeted by one of these taunts:

- "Don't cactus about it!"
- "Hop luck next time!"
- "You got schooled, bunny!"
- "That's gonna make you hopping mad!"
- "Bounce back!"
- "The sands claim another..."
- "Not so burrow now!"
- "That one hurt, didn't it?"
- "Back to the starting hole!"
- "Carrots don't grow in the sky!"

---

## 🎨 Game Screens

### Start Screen
- Game title "DESERT HOPPER"
- Cute rabbit character preview
- "START HOPPING" button
- Control hints

### Level Transition
- Level number display
- Level name/tip for the upcoming level
- Automatic transition after 1.5 seconds

### Death Screen
- Sandy dust overlay
- Death counter
- Random taunt message
- Auto-respawn after 1 second

### Win Screen
- "THE RABBIT ESCAPED!" victory banner
- Total death count
- Performance rating
- "PLAY AGAIN" button

---

## 🛠️ Technical Details

- **Engine**: Pure HTML5 Canvas
- **Language**: Vanilla JavaScript
- **No Dependencies**: Runs directly in browser
- **Responsive**: Adapts to screen size
- **Mobile Support**: Touch controls included

---

## 🌐 Hosting

This game can be hosted on various platforms:

### Versal (Recommended)
1. Upload the `index.html` file to your Versal project
2. No additional configuration needed
3. Game runs entirely client-side

### GitHub Pages
1. Go to repository Settings
2. Navigate to Pages
3. Select source branch (main)
4. Your game will be live at `https://yourusername.github.io/AziMeow_The_rabbit_game`

### Local Development
Simply open `index.html` in any modern browser.

---

## 📁 Project Structure

```
AziMeow_The_rabbit_game/
├── index.html          # Main game file (HTML5 Canvas)
├── README.md           # This documentation
└── LICENSE             # MIT License
```

---

## 🤝 Contributing

Feel free to:
- Fork this repository
- Create issues for bugs or suggestions
- Submit pull requests
- Share your high scores!

---

## 📜 License

MIT License - Feel free to use, modify, and distribute!

---

## 👨‍💻 Author

Created with ❤️ by **AzimPial**

---

## 🎮 Quick Start

1. Open `index.html` in your browser
2. Click "START HOPPING"
3. Use arrow keys to move and space to jump
4. Reach the exit in each level without touching cacti!
5. Complete all 10 levels

**Good luck, hopper!** 🐰🏜️

---

*Last updated: May 2026*
